            ...
            withCredentials([sshUserPrivateKey(
                ...
                sh """
                    ...
                    ssh -o StrictHostKeyChecking=no -i \${SSH_KEY} \${SSH_USERNAME}@\${env.HOST_IP} '
                        ...
                        echo -e "\\\\n=== Website Tests ==="
                        echo -n "CampusHostels (FE): "
                        curl -s -o /dev/null -w "%{http_code}" https://campushostels.duckdns.org/
                        echo ""
                        echo -n "BookShelf (FE): "
                        curl -s -o /dev/null -w "%{http_code}" https://bookshelfgh.duckdns.org/
                        echo ""
                        echo -n ".NET API (BE): "
                        curl -s -o /dev/null -w "%{http_code}" https://campushostels.duckdns.org/api/
                        echo ""
                        
                        echo -e "\\\\n=== Application URLs ==="
                        echo "🌐 CampusHostels: https://campushostels.duckdns.org/"
                        echo "📚 BookShelf: https://bookshelfgh.duckdns.org/"
                        echo "🔧 Django Admin: https://campushostels.duckdns.org/admin/"
                        echo "⚙️ .NET API: https://campushostels.duckdns.org/api/"
                    '
                """
            }
        }
