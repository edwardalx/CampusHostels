        ...
        withCredentials([sshUserPrivateKey(
            ...
            sh """
                ...
                ssh -o StrictHostKeyChecking=no -i \${SSH_KEY} \${SSH_USERNAME}@\${env.HOST_IP} '
                    ...
                    echo "✅ Frontend build found at: \${env.FRONTEND_DIR}/dist/"
                    echo "Build size: \$(du -sh \${env.FRONTEND_DIR}/dist/)"
                    
                    # Determine nginx directory
                    NGINX_DIR="/var/www/campushostels-fe"
                    
                    echo "Deploying to nginx directory: \$NGINX_DIR"
                    ...
                    # Copy files
                    echo "Copying files..."
                    sudo cp -r \${env.FRONTEND_DIR}/dist/* \$NGINX_DIR/
                    ...
                    sudo ls -la \$NGINX_DIR/ | head -10
                    
                    # Restart nginx if needed
                    echo "Restarting nginx..."
                    sudo systemctl restart nginx 2>/dev/null || echo "Note: nginx restart may not be needed"
                    
                    echo "✅ Deployment completed!"
                    echo "Frontend available at: https://campushostels.duckdns.org/"
                '
            """
        }
    }
